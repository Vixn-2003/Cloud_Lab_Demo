...
import { getLabs, getSubjects, getSubmissions } from '@/lib/api';
import { enrichLabs, getStudentStats } from '@/lib/data-enrichment';
...
export function DashboardContent() {
  ...
  const [labsDueSoon, setLabsDueSoon] = useState<Lab[]>([]);
  const [labsNeedingAttention, setLabsNeedingAttention] = useState<Lab[]>([]);
  const [recentFeedback, setRecentFeedback] = useState<Attempt[]>([]);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [rawLabs, subjects, submissions] = await Promise.all([
          getLabs(),
          getSubjects(),
          getSubmissions(),
        ]);

        const enriched = enrichLabs(rawLabs, submissions, subjects);
        const map = new Map(enriched.map((l) => [l.id, l]));
        setLabMap(map);

        // Current lab: in progress and has lastEditedAt, sorted by lastEditedAt desc
        ...
        setCurrentLab(inProgress[0] || null);

        // Labs due soon: not completed, sorted by due date
        const dueSoon = enriched
          .filter((l) => l.status !== 'completed' && l.status !== 'submitted' && l.dueDate)
          .sort((a, b) => new Date(a.dueDate!).getTime() - new Date(b.dueDate!).getTime())
          .slice(0, 3);
        setLabsDueSoon(dueSoon);

        // Labs needing attention: status is needs_revision or overdue
        const needingAttention = enriched.filter(
          (l) => l.status === 'needs_revision' || l.status === 'overdue'
        );
        setLabsNeedingAttention(needingAttention);

        // Recent feedback: graded attempts
        const feedback = submissions
          .filter((a) => a.status === 'graded' || a.status === 'needs_revision')
          .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
          .slice(0, 5);

        const mappedFeedback = feedback.map((f) => ({
          ...f,
          attemptNumber: f.attemptNumber || 1,
          lab_title: map.get(f.lab_id)?.title || f.lab_title || 'Lab Task',
        }));
        setRecentFeedback(mappedFeedback);

        // Student Stats
        const calculatedStats = getStudentStats(enriched, submissions);
        setStats({
          completedLabs: calculatedStats.completedLabs,
          totalLabs: calculatedStats.totalLabs,
    ...
    return (
      <div className="space-y-8 animate-pulse">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card><CardContent className="p-6 space-y-4"><Skeleton className="h-5 w-32" /><Skeleton className="h-10 w-full" /><Skeleton className="h-10 w-full" /></CardContent></Card>
          <Card><CardContent className="p-6 space-y-4"><Skeleton className="h-5 w-32" /><Skeleton className="h-10 w-full" /><Skeleton className="h-10 w-full" /></CardContent></Card>
        </div>
      </div>
    );
          ...
          {recentFeedback.length > 0 ? (
            <div className="space-y-3">
              {recentFeedback.map((attempt) => {
                const lab = labMap.get(attempt.lab_id);
