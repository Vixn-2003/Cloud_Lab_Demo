...
sequenceDiagram
    ...
    participant WS as WebSocket (Socket.IO)
    participant PTY as InteractiveTerminalService
    participant NPS as NpsLabtainerService (CLI)
    participant DK as Docker Engine (NPS Topology)
    participant DB as SQLite (submissions.db)
    ...
    rect rgb(240, 248, 255)
        note right of NPS: Khởi chạy NPS Labtainer Core
        PTY->>NPS: startLab(labId, sessionId)
        NPS->>NPS: Gọi CLI: trunk/scripts/labtainer-student/bin/labtainer <lab> -u <sessionId>
        NPS->>DK: Khởi chạy toàn bộ cụm Container của bài Labtainer (multi-node)
        PTY->>PTY: Quét tìm container sinh viên (ví dụ: nmap.attacker.sessionId)
        PTY->>PTY: Spawn node-pty đính kèm docker exec -it <attacker_container> bash
    end
    
    PTY-->>SV: Kết nối Web Terminal với container chính thức của Labtainer
    
    loop Học viên làm bài lab
        SV->>WS: Gõ lệnh thực hành (Nmap quét mạng, khai thác dịch vụ...)
        PTY->>DK: Truyền lệnh & stream I/O thời gian thực lên Xterm.js
    end
    
    SV->>WS: Gõ "exit" thoát terminal
    PTY->>PTY: Nhận tín hiệu exit
    
    rect rgb(255, 240, 245)
        note right of NPS: Luồng Chấm Điểm NPS Core (Instructor Engine)
        PTY->>NPS: stopAndGradeLab(labId, sessionId)
        NPS->>NPS: Gọi CLI: stoplab <lab> -u <sessionId> (Dừng và đóng gói kết quả)
        NPS->>NPS: Gọi CLI: gradelab <lab> -u <sessionId> (Chạy bộ chấm giảng viên)
        NPS->>NPS: Đọc tệp tin kết quả chính thức: transfer/<sessionId>.report
        NPS->>NPS: Giải mã (Parse) các tiêu chí Đạt/Không đạt & tính tổng điểm (%)
    end
    
    PTY->>DB: Lưu điểm số và chi tiết các tasks vào SQLite
    PTY-->>SV: Phát trạng thái chấm điểm thành công qua WS ("execution:status" -> finished)
```
